...
public class FileView  extends JFrame{
	...
	public FileView() {
		...
		searchBtn = new JButton("New button");
	...
	public String getSelectedFileType() {
		...
		int row = table.getSelectedRow();
		if(row!=-1) {
			result = table.getModel().getValueAt(row, 1).toString();
			//去副檔名
			result=clearType(result);
		}
		return result;
	...
	//將View上面的事件，透過controller去分配各自的工作
	...
	public void addRenameListener(ActionListener listenForRenameBtn) {
		renameBtn.addActionListener(listenForRenameBtn);
	}
	public void addUndoListener(ActionListener listenForundoBtn) {
		undoBtn.addActionListener(listenForundoBtn);
	}
	public void addRedoListener(ActionListener listenForRedoBtn) {
		redoBtn.addActionListener(listenForRedoBtn);
	}
